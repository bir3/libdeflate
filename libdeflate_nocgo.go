...
// Only use pure-go compress/flate package if explicitly requested via the build tag
// to avoid accidentially using the slow pure-go implementation
//
...
func (dd *Decompressor) GzipDecompress(outData, inData []byte) (int, error) {
	...
	actualDecompressor.Multistream(false)
